export const validateAddressUtils = (
  address: string,
  ...
  if (!address && address.length === 0) {
    errors.push(
      key === "departure" ? "Une ville de départ est requise" : "Une ville d'arrivée est requise"
    );
...
export const validateArrivalAt = (value: string, departureAt: string): string[] => {
  ...
  if (!value) {
    ...
    if (date < departureDate) {
      errors.push(
        "La date et l'heure d'arrivée ne peuvent pas être avant la date et l'heure de départ"
      );
    }
  }
  return errors;
