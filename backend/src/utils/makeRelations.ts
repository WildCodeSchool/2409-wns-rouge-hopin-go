...
function parseSelectionSet(set: SelectionSetNode, levelPossibleRelations: PossibleRelations) {
  if (set.kind === "SelectionSet" && set.selections?.length > 0) {
    for (const entry of set.selections) {
      if (entry.kind === "Field" && entry.selectionSet?.kind === "SelectionSet") {
        levelPossibleRelations[entry.name.value] = {};
        parseSelectionSet(entry.selectionSet, levelPossibleRelations[entry.name.value]);
      }
    }
  }
}
...
function checkRelation(
  ...
  relations: PossibleRelations
) {
  if (Object.keys(possibleRelations).length > 0) {
    const entityRelationsMap: Record<string, RelationMetadata[]> = {};
    for (const relation of entityRelations) {
      entityRelationsMap[relation.propertyName] = relation.inverseEntityMetadata.relations;
    }
...
export function makeRelations(info: GraphQLResolveInfo, entity: typeof BaseEntity) {
